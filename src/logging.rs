...
//! **off by default**: it is enabled only when `TREE_TUI_LOG` (a target file
...
pub fn init() -> Option<WorkerGuard> {
    let log_path = std::env::var_os("TREE_TUI_LOG").map(PathBuf::from);
    if log_path.is_none() && std::env::var_os("RUST_LOG").is_none() {
        return None;
    }

    let path = log_path.unwrap_or_else(|| PathBuf::from("tree-tui.log"));
