//! The implementation behind the `tree` binary — an interactive directory
//! visualizer for large polyglot repos.
//!
//! This library exists so the crate's own tests and benchmarks have something to
//! link against; `src/main.rs` is a thin entry point over [`run_cli`]. It is
//! **not** a general-purpose API: everything here is an internal detail of the
//! binary and carries no stability promise, so depend on it at your own risk.
//!
//! The directory is walked once into a navigable, sortable tree, then viewed
//! through swappable "lenses" (code lines, on-disk size, git churn, git status).
//! Each lens's data is computed lazily — the first time it is opened — and
...
/// Parse the command line and run whichever command it names, returning the
/// process exit code. The binary's `main` is a `#[tokio::main]` wrapper over this.
